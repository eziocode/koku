...
import { isHolidayDate, isLeaveDate, type NotificationPreferences } from "@/lib/notifications/settings";
...
function isSuppressed(prefs: NotificationPreferences, now: number): boolean {
  ...
  if (isLeaveDate(prefs.leaveDates, now)) return true;
  return false;
}
...
export function NotificationScheduler() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (
        eodPrefs.enabled &&
        !isHolidayDate(current.holidayDates, now) &&
        !isLeaveDate(current.leaveDates, now)
      ) {
