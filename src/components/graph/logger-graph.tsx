  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
        {model.nodes.length === 0 ? (
          ...
          <>
            ...
            <GraphLegend
              className="absolute left-4 top-4 max-w-[240px]"
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
        <span>
          {model.entryCount} entries · {formatDuration(Math.round(model.totalHours * 3600))} tracked
        </span>
        <span>
        ...
        {shape === "entries" && model.truncatedEntries > 0 ? (
          <span className="text-amber-600 dark:text-amber-400">
            {model.truncatedEntries} entries folded into aggregate nodes (cap reached) — narrow the
            range to see them individually.
          </span>
        ) : null}
