 ...
 * strikethrough, bulleted and numbered lists. Selecting text and clicking a
...
export function RichTextarea({ value, onChange, className, id, ...props }: RichTextareaProps) {
  ...
  function apply(mutate: (textarea: HTMLTextAreaElement) => { next: string; selectionStart: number; selectionEnd: number }) {
    const textarea = ref.current;
    if (!textarea) return;

    const { next, selectionStart, selectionEnd } = mutate(textarea);
    onChange(next);
