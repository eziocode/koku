...
export function AdminClient() {
  ...
  return <div className="space-y-8">
    ...
    <AdminGroups groups={groups} users={users} onChanged={load} canManage={canManageAdmins} />
...
function AdminGroups({ groups, users, onChanged, canManage }: { groups: AdminGroup[]; users: AdminUser[]; onChanged: () => Promise<void>; canManage: boolean }) {
  const [name, setName] = useState("");
  const [members, setMembers] = useState<Record<string, string[]>>(() => Object.fromEntries(groups.map((group) => [group.id, group.userIds])));
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => Object.fromEntries(groups.map((group) => [group.id, true])));
  ...
  useEffect(() => {
    ...
    setExpanded((current) => Object.fromEntries(groups.map((group) => [group.id, current[group.id] ?? true])));
  }, [groups]);
  ...
  async function deleteGroup(group: AdminGroup) {
    ...
    if (response.ok) { setExpanded((current) => { const next = { ...current }; delete next[group.id]; return next; }); toast.success("Group deleted."); await onChanged(); } else toast.error(await errorMessage(response, "Group deletion failed."));
  }
