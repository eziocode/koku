...
export function AdminClient() {
  ...
  return <div className="space-y-8">
    ...
    <AdminGroups groups={groups} users={users} onChanged={load} canManage={canManageAdmins} onDeleteGroup={deleteGroup} />
...
function AdminGroups({ groups, users, onChanged, canManage, onDeleteGroup }: { groups: AdminGroup[]; users: AdminUser[]; onChanged: () => Promise<void>; canManage: boolean; onDeleteGroup: (group: AdminGroup) => Promise<void> }) {
  const [name, setName] = useState("");
  const [members, setMembers] = useState<Record<string, string[]>>(() => Object.fromEntries(groups.map((group) => [group.id, group.userIds])));
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => Object.fromEntries(groups.map((group) => [group.id, false])));

  useEffect(() => {
    // Props may change after another admin saves/deletes a group.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setMembers(Object.fromEntries(groups.map((group) => [group.id, group.userIds])));
    setExpanded((current) => Object.fromEntries(groups.map((group) => [group.id, current[group.id] ?? false])));
  }, [groups]);
  ...
  async function saveGroup(group: AdminGroup) {
    ...
    if (response.ok) { setExpanded((current) => ({ ...current, [group.id]: false })); toast.success("Group members saved."); await onChanged(); } else toast.error(await errorMessage(response, "Group update failed."));
  }

  async function deleteGroup(group: AdminGroup) { await onDeleteGroup(group); }

  return <Card><CardHeader><CardTitle>Groups</CardTitle><CardDescription>{canManage ? "Organize users into reusable groups. Grouping does not change permissions." : "Workspace groups and members."}</CardDescription></CardHeader><CardContent className="space-y-4">{canManage ? <div className="flex max-w-xl gap-2"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="New group name" onKeyDown={(event) => { if (event.key === "Enter") void createGroup(); }} /><Button disabled={!name.trim()} onClick={() => void createGroup()}>Create group</Button></div> : null}{groups.length ? <div className="grid gap-4 md:grid-cols-2">{groups.map((group) => { const isExpanded = expanded[group.id] === true; return <div key={group.id} className="rounded-xl border p-4"><div className="flex items-center justify-between gap-2"><button type="button" className="min-w-0 text-left" onClick={() => setExpanded((current) => ({ ...current, [group.id]: !isExpanded }))}><p className="font-medium">{group.name}</p><p className="text-xs text-muted-foreground">{(members[group.id] ?? []).length} members · {isExpanded ? "Close" : canManage ? "Edit members" : "Show members"}</p></button>{canManage ? <div className="flex gap-2"><Button size="sm" onClick={() => void saveGroup(group)} disabled={!isExpanded}>Save</Button><Button size="icon" variant="ghost" aria-label={`Delete group ${group.name}`} onClick={() => void deleteGroup(group)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div> : null}</div>{isExpanded ? <div className="mt-3 max-h-56 space-y-2 overflow-auto">{users.map((user) => { const selected = (members[group.id] ?? []).includes(user.id); return <label key={user.id} className="flex items-start gap-2 text-sm"><input type="checkbox" checked={selected} disabled={!canManage} onChange={() => setMembers((current) => ({ ...current, [group.id]: selected ? (current[group.id] ?? []).filter((id) => id !== user.id) : [...(current[group.id] ?? []), user.id] }))} /><span>{user.displayName || "Unnamed user"}<span className="ml-1 text-xs text-muted-foreground">({user.id})</span><span className="block text-xs text-muted-foreground">{user.email}</span></span></label>; })}</div> : null}</div>; })}</div> : <p className="text-sm text-muted-foreground">No groups yet.</p>}</CardContent></Card>;
}
