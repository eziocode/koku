...
import { formatDate, formatDuration, groupRowsByUser, plainTextToTiptap, tiptapToPlainText, type AdminRow, type AdminStats, type AdminUser } from "@/lib/admin-data";
...
export function AdminClient() {
  ...
  async function load() {
    ...
    if (response.ok) { const body = await response.json() as { data: Record<string, AdminRow[]>; users?: AdminUser[]; admins?: AdminUser[]; stats?: Record<string, AdminStats>; canManageAdmins?: boolean }; setData(body.data); setUsers(body.users ?? []); setAdmins(body.admins ?? []); setStats(body.stats ?? {}); setCanManageAdmins(body.canManageAdmins === true); }
    setLoading(false);
  }
  ...
  return <div className="space-y-8">
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" /> Data explorer</CardTitle><CardDescription>{selected ? `${selected.displayName || selected.email} · ${visibleRows.length} visible records` : `${userGroups.length} users · Select user to inspect records`}</CardDescription>{selected && stats[selected.id] ? <StatsHeader stats={stats[selected.id]} /> : null}</CardHeader><CardContent className="space-y-4">
